...
import register_voter as regV
import admFunc as adFunc
from tkinter import *
from register_voter import *
from admFunc import *
...
def AdminHome(root,frame1,frame3):
    ...
    Label(frame1, text="Admin", font=('Inter', 25, 'bold')).grid(row = 0, column = 1)
    ...
    #Voter Login
    register_voter = Button(frame1, text="Register Voter", width=15, command = lambda: regV.Register(root, frame1))
    ...
    register_voter.grid(row = 5, column = 1, columnspan = 2)
...
def AdmLogin(root,frame1):
    ...
    Label(frame1, text="Admin Login", font=('Inter', 18, 'bold')).grid(row = 0, column = 2, rowspan=1)
    Label(frame1, text="").grid(row = 1,column = 0)
